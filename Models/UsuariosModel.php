...
class UsuariosModel extends Query{
    ...
    {
        $sql = "INSERT INTO detalle_permiso(id_usuario, id_permiso) VALUES (?,?)";
    ...
    {
        $sql = "DELETE FROM detalle_permiso WHERE id_usuario = ?" ;
        $datos = array($id_user);
        $data = $this->save($sql, $datos);
        if ($data == 1) {
            $res ='ok';
    ...
    {
        $sql = "SELECT * FROM detalle_permiso WHERE id_usuario = $id_user ";
        $data = $this->selectAll($sql);
        return $data;
