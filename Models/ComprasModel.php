...
class ComprasModel extends Query
{
    ...
    public function registrarDetallePrecios(int $id_producto, int $id_usuario, int $cantidad)
    {

        $sql = "INSERT INTO  detallesolicitud(id_producto, id_usuario , cantidad ) VALUES (?,?,?)";
        $datos = array($id_producto, $id_usuario, $cantidad);
        $data = $this->save($sql, $datos);
        if ($data == 1) {
            $res = "ok";
        } else {
    ...
    public function getDetallePrecios(int $id)
    {

        $sql = "SELECT d.*, p.id AS id_pro, p.descripcion FROM detallesolicitud d INNER JOIN productos p  ON d.id_producto = p.id WHERE d.id_usuario = $id";
        $data = $this->selectAll($sql);
        return $data;
    }
    ...
    public function consultarDetallePrecios(int $id_producto, int $id_usuario)
    {

        $sql = "SELECT * FROM detallesolicitud WHERE id_producto = $id_producto AND id_usuario = $id_usuario";
        $data = $this->select($sql);
        return $data;
    }
